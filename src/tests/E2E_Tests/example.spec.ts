...
test.describe('Mark all as completed', () => {
  test.beforeEach(async ({ page }) => {
    await allure.step('Create default todos and check number of todos in local storage', async () => {
      await createDefaultTodos(page);
      await checkNumberOfTodosInLocalStorage(page, 3);
    });
  });
  ...
  test('should allow me to clear the complete state of all items', async ({ page }) => {
    await allure.step('Check and then immediately uncheck "Mark all as complete"', async () => {
  ...
  test('complete all checkbox should update state when items are completed / cleared', async ({ page }) => {
    ...
    await allure.step('Uncheck first todo and ensure "Mark all as complete" is not checked', async () => {
      const firstTodo = page.getByTestId('todo-item').nth(0);
      await firstTodo.getByRole('checkbox').uncheck();
      await expect(page.getByLabel('Mark all as complete')).not.toBeChecked();
    });

    await allure.step('Check first todo again and ensure "Mark all as complete" is checked', async () => {
      const firstTodo = page.getByTestId('todo-item').nth(0);
...
test.describe('Item', () => {
  ...
  test('should allow me to mark items as complete', async ({ page }) => {
    await allure.step('Create a new todo locator and add two items', async () => {
      const newTodo = page.getByPlaceholder('What needs to be done?');
...
test.describe('Editing', () => {
  ...
  test('should hide other controls when editing', async ({ page }) => {
    ...
    await allure.step('Check that other controls are hidden when editing', async () => {
      const todoItem = page.getByTestId('todo-item').nth(1);
...
test.describe('Counter', () => {
  test('should display the current number of todo items', async ({ page }) => {
    await allure.step('Create a new todo locator and a todo count locator', async () => {
      const newTodo = page.getByPlaceholder('What needs to be done?');
      const todoCount = page.getByTestId('todo-count');
    });
...
test.describe('Clear completed button', () => {
  ...
  test('should display the correct text', async ({ page }) => {
    ...
    await allure.step('Verify that the "Clear completed" button is visible', async () => {
  ...
  test('should remove completed items when clicked', async ({ page }) => {
    ...
    await allure.step('Verify that the completed items are removed', async () => {
      const todoItems = page.getByTestId('todo-item');
  ...
  test('should be hidden when there are no items that are completed', async ({ page }) => {
    await allure.step('Check the first todo item and click "Clear completed" button', async () => {
      await page.locator('.todo-list li .toggle').first().check();
      await page.getByRole('button', { name: 'Clear completed' }).click();
    });

    await allure.step('Verify that the "Clear completed" button is hidden', async () => {
...
test.describe('Persistence', () => {
  test('should persist its data', async ({ page }) => {
    ...
    await allure.step('Check the first todo item and verify its class', async () => {
      const todoItems = page.getByTestId('todo-item');
...
test.describe('Routing', () => {
  ...
  test('should allow me to display active items', async ({ page }) => {
    await allure.step('Check the second todo item and verify active items', async () => {
  ...
  test('should respect the back button', async ({ page }) => {
    await allure.step('Check the second todo item and navigate through filters', async () => {
      const todoItem = page.getByTestId('todo-item');
      await page.getByTestId('todo-item').nth(1).getByRole('checkbox').check();
      await checkNumberOfCompletedTodosInLocalStorage(page, 1);
  ...
  test('should allow me to display completed items', async ({ page }) => {
    await allure.step('Check the second todo item and verify completed items', async () => {
      await page.getByTestId('todo-item').nth(1).getByRole('checkbox').check();
      await checkNumberOfCompletedTodosInLocalStorage(page, 1);
      await page.getByRole('link', { name: 'Completed' }).click();
  ...
  test('should allow me to display all items', async ({ page }) => {
    await allure.step('Check the second todo item and verify all items', async () => {
      await page.getByTestId('todo-item').nth(1).getByRole('checkbox').check();
      await checkNumberOfCompletedTodosInLocalStorage(page, 1);
      await page.getByRole('link', { name: 'Active' }).click();
      await page.getByRole('link', { name: 'Completed' }).click();
  ...
  test('should highlight the currently applied filter', async ({ page }) => {
    await allure.step('Verify "All" filter is selected by default', async () => {
      await expect(page.getByRole('link', { name: 'All' })).toHaveClass('selected');
    });

    await allure.step('Create locators for active and completed links and verify their selection', async () => {
